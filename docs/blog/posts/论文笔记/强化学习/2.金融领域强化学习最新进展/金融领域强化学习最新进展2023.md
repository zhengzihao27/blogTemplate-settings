...
投资组合优化问题，通常有两类问题，一类是的 Infinite time horizon 的 MDPs，
另一类则是 finite time horizon 的MDPs。
前者主要研究长期的投资策略，如养老金问题。
而后者主要研究短期内的交易，如资产购买或清算的最优执行问题，
...
MDPs With infinite-time horizon，假设奖励 $r$ 和转移动态 $P$ 是时间齐次(time homogeneous)的，
这在马尔可夫决策过程文献中是一个标准假设。
此外，如果考虑最小化成本而不是最大化奖励，问题的设置本质上是相同的。

动态规划原则(dynamic programming principle, DPP)指的是最优策略可以通过最大化
单步的奖励，然后从新的状态进行最优化处理。
我们可以用它来推导值函数(1)的贝尔曼方程

\begin{equation}
    V^{*}(s)=\sup _{a \in \mathcal{A}}\left\{\mathbb{E}[r(s, a)]+\gamma \mathbb{E}_{s^{\prime} \sim P(s, a)}\left[V^{*}\left(s^{\prime}\right)\right]\right\} .
\end{equation}

引入 Q-function ，我们可以将值函数改写成

\[V^{*}(s)=\sup _{a \in \mathcal{A}} Q^{*}(s, a) ,\]

Q-function 是RL中的其中一个基本量，定义如下：
...
:   为了最大化时间积累的奖励，agent 基于过去的经验进行学习并选择行动(exploitation)和/或尝试新的选择(Exploration)
探索提供了能将当前次优解改进为最终全局最优解的机会，
但这需要耗费时间和计算资源，这是因为过度探索可能会削弱算法收敛到最优解的速度。
与之相比的是，纯利用，即仅基于过去经验短视地选择当前解，
虽然易于实现，却往往导致次优的全局解。
因此，在强化学习算法的设计中，为提升学习和优化表现，适当平衡探索与利用至关重要。
